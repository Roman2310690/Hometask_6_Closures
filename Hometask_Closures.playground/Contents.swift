...
getClosure(closure: { print("closure") })
...
var arrayInt: [Int] = [1, 6, 5, 4, 9]
var arrayDouble: [Double] = [1.25, 4.56, 6,54, 9.23, 6.24]
var arrayString: [String] = ["qw", "qwer", "qwert", "qwertyu", "qwerty"]

arrayInt.sorted(by: { (first: Int, second: Int) -> Bool in
    return first < second
})
...
arrayInt.sorted(by: { (first: Int, second: Int) in first < second })

arrayInt.sorted(by: { (first: Int, second: Int) in first > second })

arrayInt.sorted(by: { $0 < $1 })
...
arrayDouble.sorted(by: { (first: Double, second: Double) -> Bool in
    return first < second
})
...
arrayDouble.sorted(by: { (first: Double, second: Double) in first < second })

arrayDouble.sorted(by: { (first: Double, second: Double) in first > second })

arrayDouble.sorted(by: { $0 < $1 })

arrayDouble.sorted(by: { $0 > $1 })

arrayDouble.sorted(by: <)

arrayDouble.sorted(by: >)

arrayString.sorted(by: { (first: String, second: String) -> Bool in
    return first < second
})
...
arrayString.sorted(by: { (first: String, second: String) in first < second })

arrayString.sorted(by: { (first: String, second: String) in first > second })

arrayString.sorted(by: { $0 < $1 })

arrayString.sorted(by: { $0 > $1 })

arrayString.sorted(by: <)
...
func getClosureAndArray(arrayOfInt: [Int], closure: (_ a:Int, _ b: Int?) -> Bool) -> Int? {
    var optionalValue: Int? = nil

    for value in arrayOfInt {
        if closure(value, optionalValue) {
            optionalValue = value
            }
    }
...
let numbers: [Int] = [3, 5, 8, 1, 9, 7]

let maxElement = getClosureAndArray(arrayOfInt: numbers) { $1 == nil || $0 > $1! }
let minElement = getClosureAndArray(arrayOfInt: numbers) { $1 == nil || $0 < $1! }
...
var characterOfText = [String]()
for item in vereVeryLongText {
    characterOfText.append(String(item))
}

func sortedFunc(string: String) -> Int {
...
let resultCharacterOfText = characterOfText.sorted {
    switch (sortedFunc(string: $0), sortedFunc(string: $1)) {
...
let newDictionary = names.reduce(into: [:]) { dictionary, name in
  dictionary[name.count] = String(name.sorted(by: <)).trimmingCharacters(in: .whitespaces)
}
print(newDictionary)
